...
int init_synth_controller(SynthController *synth_ctl, synthInitData init_data)
{
  ...
  synth_ctl->synth_cdata.max_amplitude = QUARTER_AMPLITUDE;
...
void dispatch_note_on(SynthController *synth_ctl, MidiMsg* msg)
{
  ...
  if (velocity > 0) {
    ...
    note->isOn = 1;
    fprintf(stdout, "NOTE ON: amplitude %d, attack %d\n", note->peak_amplitude, note->attack_buf_rem);
...
{
  double amplitude_f = 2 * (t/note->freq - floor(0.5 + t/note->freq));
